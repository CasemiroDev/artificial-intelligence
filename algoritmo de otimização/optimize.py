import six
import sys
sys.modules['sklearn.externals.six'] = six
import sklearn
import mlrose
import numpy as np


products = [('Refrigerador A',0.751,999.90),
            ('Notebook A',0.00350,2499.90),
            ('Microondas C',0.0319,299.29),
            ('Notebook C',0.527,3999.00),
            ('Celular',0.0000899,2199.12),
            ('Ventilador',0.496,199.90),
            ('Refrigerador B',0.635,849.00),
            ('Tv 55',0.400,4346.99),
            ('Microondas A',0.0424,308.66),
            ('Refrigerador C',0.870,1199.89),
            ('TV 50',0.290,3999.00),
            ('TV 42',0.200,2999.00),
            ('Microondas B',0.0544,429.90),
            ('Notebook B',0.498,1999.00)]

espaco_disponivel = 3

def print_solve(solve):
    for i in range(len(solve)):
        if solve[i] == 1:
            print(products[i][0],'  -  ',products[i][2])

def fitness_function(solve):
  custo = 0
  soma_espaco = 0
  for i in range(len(solve)):
    if solve[i] == 1:
      custo += products[i][2]
      soma_espaco += products[i][1]
  if soma_espaco > espaco_disponivel:
    custo = 1
  return custo
fitness = mlrose.CustomFitness(fitness_function)
problem = mlrose.DiscreteOpt(length= 14, fitness_fn= fitness, maximize= True, max_val= 2)

### Hill Climb na prática
#melhor_solucao, melhor_custo = mlrose.hill_climb(problem)
#melhor_solucao, melhor_custo

#print_solve(melhor_solucao)
#print(f'{melhor_custo:.2f}')


### Simulated Annealing na prática
#melhor_solucao, melhor_custo = mlrose.simulated_annealing(problem)
#melhor_solucao, melhor_custo

#print_solve(melhor_solucao)
#print(f'{melhor_custo:.2f}')


###Algoritmo Genético na prática
melhor_solucao, melhor_custo = mlrose.genetic_alg(problem, pop_size=500,mutation_prob=0.2)
melhor_solucao,melhor_custo

print_solve(melhor_solucao)
print(f'{melhor_custo:.2f}')